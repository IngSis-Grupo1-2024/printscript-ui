...
export const SnippetDetail = (props: SnippetDetailProps) => {
    ...
    const [snippetRunningOutput, setSnippetRunningOutput] = useState<string>("")
    ...
    const {mutate: runSnippet, isLoading: isRunningLoading, data: result} = useRunSnippet(props.id);
    ...
    useEffect(() => {
        if (result) {
            setSnippetRunningOutput(result)
        }
    }, [result, isRunningLoading]);
    ...
    return (
        <Box p={4} minWidth={'60vw'}>
            ...
            </Box>
            {
                isLoading ? (<>
                    ...
                    <Box display="flex" flexDirection="row" gap="8px" padding="8px">
                        ...
                        <Tooltip title={isRunningLoading ? "Stop run" : "Run"}>
                            <IconButton onClick={() => runSnippet(props.id)}>
                                {isRunningLoading ? <StopRounded/> : <PlayArrow/>}
                            </IconButton>
                        </Tooltip>
                    ...
                    <Box pt={1} flex={1} marginTop={2}>
                        ...
                        <SnippetExecution snippetRunningOutput={snippetRunningOutput}/>
                    </Box>
                </>
            }
            ...
            <DeleteConfirmationModal open={deleteConfirmationModalOpen}
                                     onClose={() => setDeleteConfirmationModalOpen(false)} id={snippet?.id ?? ""}
